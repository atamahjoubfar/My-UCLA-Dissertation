...
\author{~Aliakbar~Homayoun,~\IEEEmembership{Student Member,~IEEE,}
        and~Behzad~Razavi,~\IEEEmembership{Fellow,~IEEE}% <-this % stops a space
...
% The paper headers
\markboth{IEEE Transactions on Circuits and Systems--I: Regular Papers.}%
...
\IEEEPARstart{T}{he} phase noise of the phase/frequency detector (PFD) in a phase-locked loop (PLL) directly adds to that of the reference,
manifesting itself for a high frequency multiplication factor and/or a wide loop bandwidth.

This paper investigates the phase noise mechanisms in PFDs and computes the phase noise spectral density due to both white noise and flicker
...
\begin{figure}[htb]
\vspace{2.9in}
\special{psfile=Figures/fig1a hoffset=-50 voffset=15 vscale=90 hscale=90}
\vspace{2.3in}
\caption{(a) NOR-based PFD, and (b) output phase noise of a 5-GHz PLL due to PFD.}
\special{psfile=Figures/fig1b.eps hoffset=-50 voffset=-90 vscale=54 hscale=60}
\label{pfdnor}
\end{figure} 
...
\special{psfile=Figures/fig2 hoffset=-50 voffset=15 vscale=90 hscale=90}
\label{pll}
\end{figure}
...
\special{psfile=Figures/fig3 hoffset=-50 voffset=15 vscale=90 hscale=90}
\label{3cases}
\end{figure}
...
\special{psfile=Figures/fig4 hoffset=-50 voffset=15 vscale=90 hscale=90}
\label{noisepercycle}
\end{figure}
...
\special{psfile=Figures/fig5 hoffset=-50 voffset=15 vscale=90 hscale=90}
\label{noiseenvelope}
\end{figure}
...
We now wish to approximate the area under the noise envelopes by a simple function. As shown in Fig. \ref{deltat}, the flicker noise envelope is
...
\special{psfile=Figures/fig6 hoffset=-50 voffset=15 vscale=90 hscale=90}
\label{deltat}
\end{figure}
...
Based on the foregoing approximations and utilizing the rectangular function, $w(t)$, in Fig. \ref{deltat}, we now outline the inverter phase noise
...
\special{psfile=Figures/fig7 hoffset=-50 voffset=15 vscale=90 hscale=90}
\label{vni}
\end{figure}
...
\special{psfile=Figures/fig8 hoffset=-50 voffset=15 vscale=90 hscale=90}
\label{nand}
\end{figure}
...
\special{psfile=Figures/fig9 hoffset=-50 voffset=25 vscale=90 hscale=90}
\label{pfdnand}
\end{figure}
...
We must also follow the $\overline{\rm Up}$ and $\overline{\rm Down}$ rising edges through the reset path. As illustrated in Fig.
...
\special{psfile=Figures/fig10 hoffset=-50 voffset=15 vscale=90 hscale=90}
\label{pfdtspc}
\end{figure}
...
\special{psfile=Figures/fig11.eps hoffset=-50 voffset=-120 vscale=60 hscale=60}
\label{inverterchain}
\end{figure} 
\begin{figure}[hbt]
\vspace{2.8in}
\caption{Phase noise of a chain of eight NANDs running at 1 GHz (with one input tied to $V_{DD}$).}
\special{psfile=Figures/fig12.eps hoffset=-50 voffset=-110 vscale=60 hscale=60}
\label{nandchain}
\end{figure} 
...
Illustrated in Fig. \ref{pfdoptim} are the results of the optimization procedure described in Section VI. For a given power consumption, the phase noise is
reduced by 4 to 8 dB for the two PFDs.

\begin{figure}[hbt]
\vspace{2.8in}
\caption{Phase noise of NAND PFD at various input frequencies.}
\special{psfile=Figures/fig13.eps hoffset=-50 voffset=-120 vscale=60 hscale=60}
\label{pfdnandsim}
\end{figure} 
\begin{figure}[hbt]
\vspace{2.8in}
\caption{Phase noise of TSPC PFD at various input frequencies.}
\special{psfile=Figures/fig14.eps hoffset=-50 voffset=-120 vscale=60 hscale=60}
\label{pfdtspcsim}
\end{figure} 
\begin{figure}[hbt]
\vspace{2.8in}
\caption{Phase noise of NAND and TSPC PFDs before and after optimization.}
\special{psfile=Figures/fig15.eps hoffset=-50 voffset=-120 vscale=60 hscale=60}
\label{pfdoptim}
\end{figure} 
...
\special{psfile=Figures/fig16 hoffset=-50 voffset=15 vscale=90 hscale=90}
\label{ppmcompare}
\end{figure}
...
\special{psfile=Figures/fig17 hoffset=-50 voffset=25 vscale=90 hscale=90}
\label{squaretonepn}
\end{figure}
...
\special{psfile=Figures/fig18 hoffset=-50 voffset=15 vscale=90 hscale=90}
\label{psdrising}
\end{figure}
...
Considering only the $\rm sinc^2$ shape itself, we recognize that the inverse Fourier transform of ${\Delta T}^2 {\rm sinc^2}(\pi f \Delta T)$ is a triangle,
...
\special{psfile=Figures/fig19 hoffset=-50 voffset=15 vscale=90 hscale=90}
\label{sinc2}
\end{figure}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip]{photo.eps}}]{Aliakbar Homayoun}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip]{BRpic.eps}}]{Behzad Razavi}
